...
class EditTastingNoteBloc implements Bloc {
  EditTastingNoteBloc(this.editTastingNoteModel) {
    _subscriptions.add(onBuild.listen((_) {
      final note = editTastingNoteModel.createTastingNote();
      _tastingNoteSubject.add(note);
    }));
    // _subscriptions
  ...
  final BehaviorSubject<TastingNote> _tastingNoteSubject = BehaviorSubject();
  ...
  Observable<TastingNote> get tastingNote => _tastingNoteSubject;
  Sink<TastingNote> get updateTastingNote => _tastingNoteSubject;
  ...
  void dispose() async {
    ...
    await onBuild.close();
    await _tastingNoteSubject.close();
